...
fn unique_output_root(manifest: &Path) -> Result<PathBuf, std::time::SystemTimeError> {
    ...
    Ok(manifest
        .join("../target/shapeic-ota-4t-electrical-verification")
...
mod tests {
    ...
    fn real_template_exports_four_scalar_metrics_and_total_widths() {
        ...
        assert!(netlist.contains(&format!("w={} ng=2", format_float(diff_width))));
        assert!(netlist.contains(&format!("w={} ng=2", format_float(mirror.total_width))));
